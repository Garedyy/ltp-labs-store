...
- **Addendum (2026-09-13, #40)**: the "Browse the shop" link is the main call to action of the
  landing page, so it is rendered between the intro paragraph and the trending grid, aligned to
  the end of its row (visible without scrolling down to 320 px), never after the cards;
  `home.spec.ts` and `no-js.spec.ts` assert the DOM order.
