...
| search-param change on `/search` (first search included; `CatalogueResults` stays mounted across the prompt and results states)       | `catalogue.search.announce`                                           | unchanged (page change: the clicked pagination link, or the current page link when it vanished; no scroll, D-21)                                                 |
...
| search-param change on `/shop` (sort, category, page; `CatalogueResults`, once `navigation.state` is idle)                            | `catalogue.results.announce`                                          | unchanged (page change: the clicked pagination link, or the current page link when it vanished; no scroll, D-21)                                                 |
