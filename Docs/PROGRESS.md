...
- **Current branch**: `feature/a11y-audit` (PR open → `development`)
- **Current step**: branch 12 complete; local criteria verified; waiting for CI, then squash-merge
- **Next action**: after merge, branch `feature/performance` (plan §4 branch 13): verify
  preload/preconnect/image priorities, `@react-router/serve` headers, bundle sizes (catalogue
  route JS < 90 KB gzipped, CSS < 15 KB) recorded in the README, Lighthouse mobile run
- **Open questions for the user**: none (but see "Incidents": PR #11 was merged with a red e2e
  job; fixed in branch 12)
