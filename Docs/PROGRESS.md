...
- **Current branch**: `feature/search` (PR open → `development`)
- **Current step**: branch 8 complete; local criteria verified; waiting for CI, then squash-merge
- **Next action**: after merge, branch `feature/product-detail` (plan §4 branch 9): product route
  loader (`ProductView`: formatted prices/discount/rating/dates, `?image` clamp,
  `shouldRevalidate` ignoring `?image`), `ProductGallery`, `StockStatus`, `ProductInfoList`,
  `ReviewList`, `Price` + `DiscountBadge` + `Rating` usage, no Add-to-cart button yet (stated in
  PR), `product.*` translations, `product.spec.ts`
