...
- **Current branch**: `feature/cart-session` (PR open → `development`)
- **Current step**: branch 10 complete; local criteria verified; waiting for CI, then squash-merge
- **Next action**: after merge, branch `feature/cart-page` (plan §4 branch 11): cart route
  (loader `loadCartView` + flash notice + commit; action for `set-quantity | remove | apply-promo
| remove-promo | checkout`), `CartLineItem`, `QuantityStepper`, `CartSummary`, `PromoCodeForm`,
  `CheckoutActions`, `EmptyCart`, `CartNotice`, `order-confirmation.tsx`, `cart.*` translations,
  `cart.spec.ts` (+ no-JS), routes list
