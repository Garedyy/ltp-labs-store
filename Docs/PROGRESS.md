...
- **Current branch**: `development` (branches 0–6 merged; nothing in progress)
- **Current step**: ready to start branch 7
- **Next action**: `git checkout development && git pull`, then
  `git checkout -b feature/catalogue` (plan §4 branch 7): catalogue route loader
  (`getCategories` → `parseCatalogueQuery` → canonical 302 → `getProductsByCategory` /
  `getProducts` → `CatalogueView` with `formatPrice`), components `CatalogueResults`, `SortForm`
  (visible Apply, no auto-submit), `ResultsSummary` (`#results-heading`), `CategoryFilter`
  (checkbox visuals, single select, `onChange` navigate + `<noscript>` Apply, Clear filter),
  `ProductCard`, `ProductGrid`, `Pagination` (window of 5, `aria-current`), `EmptyState`;
  route-owned announcements on search-param changes; translations (`catalogue.*` incl. the 24
  category names in EN/PT); `catalogue.spec.ts` + no-JS; append routes to `tests/e2e/routes.ts`
