...
- **Current branch**: `feature/docs-release` (PR open → `development`)
- **Current step**: branch 14 complete; waiting for CI, then squash-merge, then the release:
  `development → main` merge commit `chore(release): v1.0.0`, tag `v1.0.0`, push
- **Next action**: after the release, the plan is fully executed. Remaining human-only items:
  VoiceOver run and Chrome vision-deficiency check (log them in `ACCESSIBILITY.md`).
...
- [x] checklist complete; every TO VERIFY resolved (8/8); [ ] tag `v1.0.0` pushed (after PR #14 merges)
