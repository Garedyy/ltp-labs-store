...
- **Current branch**: `feature/product-detail` (PR open → `development`)
- **Current step**: branch 9 complete; local criteria verified; waiting for CI, then squash-merge
- **Next action**: after merge, branch `feature/cart-session` (plan §4 branch 10):
  `services/cart/{session.server,types,cart,totals,promo-codes,intents,load-cart.server}.ts` +
  tests, `add` action on the product route (`AddToCartForm` with fetcher, no-JS PRG + flash
  notice), layout loader `cartCount` from the session, notice/error codes, e2e (no-JS add twice
  → badge 2, refresh does not re-add, tampered cookie → empty cart, 50-line cookie < 4000 B, 51st
  line → `cart-full`)
