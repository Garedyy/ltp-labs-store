...
- **Current branch**: `feature/dummyjson-client` (PR open → `development`)
- **Current step**: branch 6 complete; local criteria verified; waiting for CI, then squash-merge
- **Next action**: after merge, `git checkout development && git pull`, branch `feature/catalogue`
  (plan §4 branch 7): catalogue route (loader with categories → query → products → `CatalogueView`),
  `CatalogueResults`, `SortForm`, `ResultsSummary`, `CategoryFilter`, `ProductCard`, `ProductGrid`,
  `Pagination`, `EmptyState`, route-owned announcements, e2e
