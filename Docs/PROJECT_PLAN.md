...
| **Product** (`grid gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] lg:gap-12`) | gallery image box `aspect-square rounded-2xl bg-surface-placeholder object-contain` (square on phones; `md:aspect-[5/3]` from tablet — the wireframe's landscape box, as wide as its column, with the square image centred inside — #27), thumbnails `flex gap-2 overflow-x-auto size-16`; title `text-h3 font-medium`, rating, price `text-h4`, stock, Add to cart full width, description `text-body-sm`, `<dl>`, reviews | title `md:text-h2` | two columns; reviews `lg:col-span-2` |
