...
- Home: the "Browse the shop" link now sits on the heading row, at the end of the "Trending
  products" line (wrapping under it on narrow screens), instead of after the eight product cards,
  so the way to the full catalogue is visible without scrolling on desktop and on a 320 px phone;
  DOM order and visual order stay the same (#40, D-13).
